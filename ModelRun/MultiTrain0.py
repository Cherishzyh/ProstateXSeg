...
def EnsembleTrain():
    ...
    total_epoch = 10000
    batch_size = 24
    model_folder = MakeFolder(model_root + '/MultiUNet_bce')
    ...
    model = MultiSeg(1, 1, is_atten=False).to(device)
    ...
    for epoch in range(total_epoch):
        ...
        for ind, (inputs, outputs) in enumerate(train_loader):
            ...
            # preds.shape = (24, 4, 200, 200)
            preds = model(inputs)

            # slice_arr = (torch.sum(outputs[:, :, ...], dim=(2, 3)) != 0).int()
            # for slice in range(slice_arr.shape[-1]):
            #     new_preds = slice_arr[:, slice] * preds[:, slice, ...]
            #     loss_list.append(criterion(new_preds, torch.squeeze(outputs[:, slice, ...])))
        ...
        with torch.no_grad():
            for ind, (inputs, outputs) in enumerate(val_loader):
                ...
                # slice_arr = (torch.sum(outputs[:, :, ...], dim=(2, 3)) != 0).int()
                # for slice in range(slice_arr.shape[-1]):
                #     new_preds = torch.squeeze(slice_arr[:, slice]) * torch.squeeze(preds[:, slice, ...])
                #     loss_list.append(criterion(new_preds, torch.squeeze(outputs[:, slice, ...])))
                # loss = loss_list[0] + loss_list[1] + loss_list[2] + loss_list[3]
                loss = criterion(preds, outputs)
