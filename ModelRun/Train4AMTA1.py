...
from CnnTools.T4T.Utility.Loss import FocalLoss, WeightedDiceLoss
...
def Train(model, device, model_name, net_path):
    ...
    for epoch in range(total_epoch):
        ...
        for ind, (inputs, outputs) in enumerate(train_loader):
            ...
            loss4 = 1 / 12 * diceloss(region[:, 0], outputs[:, 0]) + 2 / 12 * diceloss(region[:, 1], outputs[:, 1]) + \
                    2 / 12 * diceloss(region[:, 2], outputs[:, 2]) + 3 / 12 * diceloss(region[:, 3], outputs[:, 3]) + \
                    4 / 12 * diceloss(region[:, 4], outputs[:, 4])
