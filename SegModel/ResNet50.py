...
class ResUNet(nn.Module):
    def __init__(self, block, layers, inchannels, outchannels, num_classes=1000, zero_init_residual=False,
        ...
        self.conv = nn.Conv2d(inchannels, 64, kernel_size=3, stride=1, padding=1,
                              bias=False)
        self.bn1 = norm_layer(64)
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        ...
        self.conv5 = nn.Conv2d(32, outchannels+1, 1)
...
def ModelRun(inchannel, outchannel, res_num=50, seg=False):
    if seg:
        if res_num == 34:
            return ResUNet(BasicBlock, [3, 4, 6, 3], inchannel, outchannel)
        elif res_num == 50:
            return ResUNet(Bottleneck, [3, 4, 6, 3], inchannel, outchannel)
    else:
        if res_num == 34:
            return ResNet(BasicBlock, [3, 4, 6, 3], inchannel, outchannel)
        elif res_num == 50:
            return ResNet(Bottleneck, [3, 4, 6, 3], inchannel, outchannel)


if __name__ == '__main__':
    # model = ResNet(BasicBlock, [3, 4, 6, 3], 5, 2).to(device)
    model = ResUNet(BasicBlock, [3, 4, 6, 3], 5, 2).to(device)
    inputs = torch.randn(1, 5, 192, 192).to(device)
    prediction, z = model(inputs)
    print(prediction.shape, z.shape)